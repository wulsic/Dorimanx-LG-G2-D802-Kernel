...
RAMDISK_BRANCH=$(git -C ../LG-G2-D802-Ramdisk/ commit | grep "origin/cm-kitkat-ramdisk" | wc -l);
...
if [ "$RAMDISK_BRANCH" == "1" ]; then
	...
	if [ "$RAMDISK_NOT_SAVED" != "1" ]; then
		git -C ../LG-G2-D802-Ramdisk/ checkout cm-kitkat-ramdisk;
		echo -e "\e[1;31mRamDisk Switched to cm-kitkat-ramdisk Branch!\e[m"
	else
		echo -e "\e[1;31mRamDisk has to be SAVED (commited) before switching to cm-kitkat-ramdisk Branch\e[m";
