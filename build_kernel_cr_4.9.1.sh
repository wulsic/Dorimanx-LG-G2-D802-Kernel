...
export ARCH=arm
export CROSS_COMPILE=$PARENT_DIR/../cr-arm-eabi-4.9.1/bin/arm-eabi-
...
GETVER=`grep 'Alucard-*-V' .config |sed 's/Alucard-//g' | sed 's/.*".//g' | sed 's/-C.*//g'`;
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	./mkbootimg --cmdline 'console=null androidboot.hardware=qcom user_debug=31' --kernel $PACKAGEDIR/zImage --ramdisk $PACKAGEDIR/ramdisk.gz --base 0x80200000 --pagesize 2048 --ramdisk_offset 0x02000000 --output $PACKAGEDIR/boot.img
	cd $PACKAGEDIR
	...
	FILENAME=Kernel-Alucard-${GETVER}-`date +"[%H-%M]-[%d-%m]-CM-EUR-KK4.4-SGIV-PWR-CORE"`.zip
	zip -r $FILENAME .;
